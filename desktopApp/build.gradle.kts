...
// desktopApp — JVM application module for EchoBox Desktop.
//
...
// Append EchoBox-specific keys to Conveyor's generated config file and
...
tasks.named<hydraulic.conveyor.gradle.WriteConveyorConfigTask>("writeConveyorConfig") {
    ...
    doLast {
        destination.get().asFile.appendText(
            """
            |app.fsname = echobox
            |app.display-name = EchoBox
            |app.rdns-name = com.alzimer.echobox
            |
...
compose.desktop {
    application {
        mainClass = "com.alzimer.echobox.MainKt"
                ...
                val macExtraPlistKeys =
                    ...
                    <array>
                        <dict>
                            ...
                            <string>com.alzimer.echobox.deeplink</string>
                            <key>CFBundleURLSchemes</key>
                            <array>
                                <string>echobox</string>
                            </array>
                        </dict>
                    </array>
                    """.trimIndent()
...
tasks.register("packageConveyorAppImage") {
    ...
    doLast {
        ...
        val iconDst = appDir.resolve("echobox.png")
        ...
        val versionName =
            libs.versions.version.name
                .get()
        val desktopFile = appDir.resolve("echobox.desktop")
        ...
        desktopFile.writeText(
            ...
            |Version=1.0
            |Name=EchoBox
            |Comment=EchoBox v$versionName - FOSS YouTube Music Client
            |Exec=bin/echobox %u
            |Icon=echobox
            |Terminal=false
            |Categories=Audio;AudioVideo;
            |StartupWMClass=EchoBox
            |MimeType=x-scheme-handler/echobox;x-scheme-handler/wordbyword;
            |
            """.trimMargin(),
        )
        ...
        appRun.writeText(
            ...
            |if [ ! -f "${'$'}ICON_DIR/echobox.png" ] || [ "${'$'}HERE/echobox.png" -nt "${'$'}ICON_DIR/echobox.png" ]; then
            |    mkdir -p "${'$'}ICON_DIR"
            |    cp "${'$'}HERE/echobox.png" "${'$'}ICON_DIR/echobox.png"
            ...
            |sed "s|Exec=bin/echobox|Exec=${'$'}APPIMAGE_PATH|" "${'$'}HERE/echobox.desktop" > "${'$'}DESKTOP_DIR/com-alzimer-echobox-MainKt.desktop"
        ...
        // Conveyor's launcher lives at output/bin/echobox (lowercase).
        val appExecutable = appDir.resolve("bin/echobox")
...
tasks.register("buildLinuxAppImage") {
    group = "distribution"
    description = "Full EchoBox Desktop Linux AppImage build pipeline (mpvSetupAll → conveyor → AppImage)."
...
tasks.register("buildMacZipAmd64") {
    group = "distribution"
    description = "Full EchoBox Desktop macOS Intel .zip pipeline (mpvSetupAll → conveyor)."
    dependsOn(conveyorMakeMacZipAmd64)
}

tasks.register("buildMacZipAarch64") {
    group = "distribution"
    description = "Full EchoBox Desktop macOS Apple Silicon .zip pipeline (mpvSetupAll → conveyor)."
...
tasks.register("buildWindowsMsix") {
    group = "distribution"
    description = "Full EchoBox Desktop Windows .msix pipeline (mpvSetupAll → conveyor)."
