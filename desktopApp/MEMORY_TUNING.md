...
```bash
git -C core checkout -- media/media-jvm/src/main/java/com/echobox/media_jvm/mpv/MpvPlayerAdapter.kt
rm -rf core/media/media-jvm/src/main/java/com/echobox/media_jvm/memory
...
```bash
pid=$(pgrep -f -i echobox | while read p; do [ "$(cat /proc/$p/comm)" = echobox ] && echo $p; done | head -1)
...
```powershell
Get-Process EchoBox | Select-Object Name,Id,@{n="WorkingSet_MB";e={[int]($_.WorkingSet64/1MB)}}
jcmd <pid> GC.heap_info
```
