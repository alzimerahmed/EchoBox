package com.alzimer.echobox.data.db.datasource
...
import com.alzimer.echobox.domain.data.entities.AutoEqCurveEntity
import com.alzimer.echobox.domain.data.entities.AutoEqEntryEntity
import com.alzimer.echobox.domain.data.entities.AutoEqIndexMetaEntity
import com.alzimer.echobox.domain.data.entities.EpisodeEntity
import com.alzimer.echobox.domain.data.entities.FollowedArtistSingleAndAlbum
import com.alzimer.echobox.domain.data.entities.GoogleAccountEntity
...
import com.alzimer.echobox.domain.data.entities.NotificationEntity
...
import com.alzimer.echobox.domain.data.entities.QueueEntity
...
import com.alzimer.echobox.domain.data.entities.YourYouTubePlaylistList
...
internal class LocalDataSource(
    ...
    fun getLocalFileSongs() = databaseDao.getLocalFileSongs()

    suspend fun getLocalFileSongIds() = databaseDao.getLocalFileSongIds()

    /**
     * Same batching reason as [deleteSongsAndRelatedData]: the missing-file list is bounded by the
     * library size, and SQLite caps bind parameters per statement. One transaction so the Flow
     * invalidates once rather than per chunk, and the satellite deletes sweep lyrics/format/
     * song-info rows that only the clear-history path would otherwise reach.
     */
    suspend fun deleteLocalFileSongsByIds(videoIds: List<String>): Int =
        musicDatabase.withTransaction {
            videoIds.chunked(DELETE_BATCH_SIZE).sumOf { batch ->
                val deleted = databaseDao.deleteLocalFileSongsByIds(batch)
                databaseDao.deleteLyricsByIds(batch)
                databaseDao.deleteTranslatedLyricsByIds(batch)
                databaseDao.deleteNewFormatsByIds(batch)
                databaseDao.deleteSongInfoByIds(batch)
    ...
    /**
     * Re-keys a local song row and its pair-table / playback-event references. Deferred FK checks
     * are mandatory: updating `song.videoId` while pair rows still point at the old id is an
     * immediate-FK violation mid-transaction, and reordering just moves the violation to the pair
     * update instead.
     */
    suspend fun rekeyLocalSongId(
        oldId: String,
        newId: String,
    ) = musicDatabase.withTransaction {
        musicDatabase.query("PRAGMA defer_foreign_keys = true", null).close()
        databaseDao.rekeySongId(oldId, newId)
        databaseDao.rekeyPairSongIds(oldId, newId)
        databaseDao.rekeyPlaybackEventIds(oldId, newId)
    }
