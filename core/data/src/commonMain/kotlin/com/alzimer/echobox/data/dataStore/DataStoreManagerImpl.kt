...
import com.alzimer.echobox.domain.data.model.network.ProxyConfiguration
import com.alzimer.echobox.domain.data.player.ReverbPreset
import com.alzimer.echobox.domain.manager.DataStoreManager
import com.alzimer.echobox.domain.manager.DataStoreManager.Values.AI_PROVIDER_GEMINI
import com.alzimer.echobox.domain.manager.DataStoreManager.Values.FALSE
import com.alzimer.echobox.domain.manager.DataStoreManager.Values.GITHUB
import com.alzimer.echobox.domain.manager.DataStoreManager.Values.LOCAL_PLAYLIST_FILTER_OLDER_FIRST
import com.alzimer.echobox.domain.manager.DataStoreManager.Values.PROXY_TYPE_HTTP
import com.alzimer.echobox.domain.manager.DataStoreManager.Values.PROXY_TYPE_SOCKS
import com.alzimer.echobox.domain.manager.DataStoreManager.Values.REPEAT_ALL
import com.alzimer.echobox.domain.manager.DataStoreManager.Values.REPEAT_MODE_OFF
import com.alzimer.echobox.domain.manager.DataStoreManager.Values.REPEAT_ONE
import com.alzimer.echobox.domain.manager.DataStoreManager.Values.SIMPMUSIC
...
import com.alzimer.echobox.common.QUALITY as COMMON_QUALITY
...
internal class DataStoreManagerImpl(
    ...
    override val localFilesEnabled: Flow<String> =
        settingsDataStore.data.map { preferences ->
            preferences[LOCAL_FILES_ENABLED] ?: FALSE
        }

    override suspend fun setLocalFilesEnabled(enabled: Boolean) {
        withContext(Dispatchers.IO) {
            settingsDataStore.edit { settings ->
                settings[LOCAL_FILES_ENABLED] = if (enabled) TRUE else FALSE
            }
        }
    }

    override val localFilesLastScan: Flow<Long> =
        settingsDataStore.data.map { preferences ->
            preferences[LOCAL_FILES_LAST_SCAN] ?: 0L
        }

    override suspend fun setLocalFilesLastScan(time: Long) {
        withContext(Dispatchers.IO) {
            settingsDataStore.edit { settings ->
                settings[LOCAL_FILES_LAST_SCAN] = time
            }
        }
    }
