...
actual class Extractor {
    ...
    actual fun mergeAudioVideoDownload(filePath: String): DownloadProgress {
        val command =
            listOf(
                ...
                "1:a:0",
                "-shortest",
                "$filePath-EchoBox.mp4",
            ).joinToString(" ")

        if (FileSystem.SYSTEM.exists("$filePath-EchoBox.mp4".toPath())) {
            FileSystem.SYSTEM.delete("$filePath-EchoBox.mp4".toPath())
        }
    ...
    actual fun saveAudioWithThumbnail(
        ...
        try {
            ...
            if (FileSystem.SYSTEM.exists("$filePath-echobox.mp3".toPath())) {
                FileSystem.SYSTEM.delete("$filePath-echobox.mp3".toPath())
            }
        } catch (e: IOException) {
            e.printStackTrace()
        }
