...
/**
 * The YouTube stream format ids (`itag`) EchoBox asks for, in one place.
 *
...
object MEDIA_NOTIFICATION {
    ...
    const val NOTIFICATION_CHANNEL_NAME = "EchoBox Playback Notification"
    const val NOTIFICATION_CHANNEL_ID = "EchoBox Playback Notification ID"
}
...
/**
 * videoId prefix for on-device audio files found through MediaStore. Local tracks live in the
 * `song` table like streamed tracks so likes, playlists and play counts work unchanged; the
 * media source factory detects the prefix and serves the file instead of resolving a stream.
 */
const val LOCAL_FILE_ID_PREFIX = "local_"

/**
 * Pseudo playlistId for a queue built from the "Local files" screen. Must NOT start with
 * [LOCAL_PLAYLIST_ID]: that prefix makes the media handler parse a numeric local-playlist row
 * id off the suffix.
 */
const val LOCAL_FILES_SOURCE_ID = "LOCAL_FILES"
