 ...
 * - echobox://open-app?url=<encoded_url>  (redirected from website)
 * - echobox://watch?v=VIDEO_ID            (direct scheme)
 * - echobox://playlist?list=PLAYLIST_ID   (direct scheme)
 * - echobox://channel/CHANNEL_ID          (direct scheme)
 * - echobox://album?id=ALBUM_ID           (direct scheme)
...
object DesktopDeepLinkHandler {
    ...
    private val pendingUriFile: File by lazy {
        File(System.getProperty("java.io.tmpdir"), "echobox_pending_deeplink.txt")
    }
     ...
     *
     * Conversion rules:
     * 1. echobox://open-app?url=<encoded_url>
     *    → Extract the `url` param and use it as intent data
     *
     * 2. echobox://watch?v=xxx, echobox://playlist?list=xxx, etc.
    ...
    private fun parseToIntent(uri: String): GenericIntent {
        ...
        val actualUri = when {
            // echobox://open-app?url=<encoded_url>
            parsed.scheme == "echobox" && parsed.host == "open-app" -> {
                val urlParam = parsed.getQueryParameter("url")
                if (urlParam != null) {
                    Logger.d(TAG, "Extracted URL from open-app: $urlParam")
                    Uri.parse(urlParam)
                } else {
                    // echobox://open-app without params → just open the app, no navigation
                    Logger.d(TAG, "open-app without URL param, just opening app")
                    null
                }
            }

            // echobox://watch?v=xxx → https://simpmusic.org/app/watch?v=xxx
            // echobox://playlist?list=xxx → https://simpmusic.org/app/playlist?list=xxx
            // echobox://channel/UCxxx → https://simpmusic.org/app/channel/UCxxx
            // echobox://album?id=xxx → https://simpmusic.org/app/album?id=xxx
            parsed.scheme == "echobox" && parsed.host != null -> {
                ...
                Logger.d(TAG, "Converted echobox:// to: $convertedUrl")
