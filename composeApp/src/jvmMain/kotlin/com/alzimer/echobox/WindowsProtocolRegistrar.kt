...
/**
 * Registers the "echobox://" custom URI protocol handler in Windows Registry
 * under HKEY_CURRENT_USER (no admin rights required).
 *
 * Registry structure:
 * ```
 * HKCU\Software\Classes\echobox
 *     (Default) = "URL:EchoBox Protocol"
 *     URL Protocol = ""
 *     \DefaultIcon
 *         (Default) = "\"<exe_path>\",0"
 *     \shell\open\command
...
object WindowsProtocolRegistrar {
    ...
    fun register() {
        ...
        register(SCHEME, "URL:EchoBox Protocol", exePath)
        register(LASTFM_SCHEME, "URL:EchoBox Last.fm Callback", exePath)
    }
    ...
    private fun isAlreadyRegistered(
        ...
        return try {
            ...
            // Registry stores path with quotes: "C:\path\to\EchoBox.exe" "%1"
    ...
    private fun resolveExePath(): String? {
        ...
        //   <app>/EchoBox.exe
        // So we go: java.home → parent (runtime) → parent (app) → EchoBox.exe
        ...
        if (appDir != null) {
            val exeFile = java.io.File(appDir, "EchoBox.exe")
            if (exeFile.exists()) {
