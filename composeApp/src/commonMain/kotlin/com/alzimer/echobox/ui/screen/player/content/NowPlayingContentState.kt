 ...
 *
 * EchoBox Lyrics is the only provider with a vote endpoint, and the vote is cast against the
 * EchoBox record itself — so the provider tag alone is NOT the condition: `simpMusicLyrics`
 * must actually be there. Either half qualifying is enough, because the dialog rates whichever
 * of the two came from EchoBox.
 *
...
/**
 * Everything a Now Playing content layer reads. The shell ([com.alzimer.echobox.ui.screen.player.NowPlayingScreenContent])
