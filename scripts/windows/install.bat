@echo off
REM EchoBox offline installer for Windows.
...
REM   - echobox.crt
REM   - echobox-<version>.msix     (any .msix matching echobox-*.msix)
REM
...
echo.
echo === EchoBox offline installer ===
echo.

REM --- 2. Install cert to Trusted People (idempotent via certutil) -----------
if not exist "%~dp0echobox.crt" (
    echo [ERROR] echobox.crt not found next to install.bat
    pause
    exit /b 1
)

echo [1/3] Importing echobox.crt to LocalMachine\TrustedPeople ...
certutil -addstore -f "TrustedPeople" "%~dp0echobox.crt"
if %errorlevel% neq 0 (
...
set "MSIX="
for %%f in ("%~dp0echobox-*.msix") do set "MSIX=%%f"

if not defined MSIX (
    echo [ERROR] No echobox-*.msix found in this folder.
    pause
    exit /b 1
)
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "try { Add-AppxPackage -Path '%MSIX%' -ForceApplicationShutdown -ForceUpdateFromAnyVersion -ErrorAction Stop; exit 0 } catch { Write-Host 'In-place update failed; removing existing EchoBox and reinstalling...'; Get-AppxPackage -Name 'Simpmusic' | Remove-AppxPackage -ErrorAction SilentlyContinue; try { Add-AppxPackage -Path '%MSIX%' -ForceApplicationShutdown -ErrorAction Stop; exit 0 } catch { Write-Host $_; exit 1 } }"
...
REM --- 4. Launch app ---------------------------------------------------------
echo.
echo [3/3] Launching EchoBox ...
start "" "shell:appsFolder\Simpmusic_ejp2bhxmz1qq6!Simpmusic"

echo.
echo EchoBox installed successfully.
echo To uninstall: Settings -^> Apps -^> Installed apps -^> EchoBox -^> Uninstall
echo.
pause
endlocal
